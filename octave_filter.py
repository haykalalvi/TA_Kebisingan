...
def octavefilter(x, fs, fraction=1, order=6, limits=None, show=0, sigbands=0,
                 mic_sensitivity=-26, mic_ref_db=94.0, mic_offset_db=3.0103, mic_bits=24):
    """
    :param x: Signal
    ...
    :param mic_sensitivity: Microphone sensitivity in dBFS (default: -26 for INMP441)
    :param mic_ref_db: Reference dB level (default: 94.0)
    :param mic_offset_db: Calibration offset (default: 3.0103)
    :param mic_bits: Valid bits in microphone data (default: 24)
    """
    
    if limits is None:
        limits = [12, 20000]
    
    # Calculate reference amplitude (sama seperti ESP32)
    MIC_REF_AMPL = pow(10, mic_sensitivity/20) * ((1 << (mic_bits-1)) - 1)
    
    # ... (kode yang lain sama sampai bagian filtering) ...
...
#     # Get SOS filter coefficients (3D - matrix with size: [freq,order,6])
    ...
    if sigbands:
        ...
        for idx in range(len(freq)):
            ...
            # PERBAIKAN: Gunakan RMS, bukan STD
            rms = np.sqrt(np.mean(y ** 2))
            spl[idx] = mic_offset_db + mic_ref_db + 20 * np.log10(rms / MIC_REF_AMPL)
